export default async (page) => {
  ...
  if (
    await page.evaluate(
      () =>
        ...
        (
          document.querySelector('h1#headingText') as HTMLElement
        ).innerHTML.includes('erify')
    )
  ) {
    try {
  ...
  await page.evaluate(() => {
    ...
    (addOnMenuButton as HTMLElement).dispatchEvent(
  ...
  await page.waitForSelector(
    'div.goog-menu.goog-menu-vertical.apps-menu-hide-mnemonics:last-child > div:nth-child(1) > div',
    { visible: true }
  );
  ...
  await page.evaluate(() => {
    ...
    (bootstrapMenuButton as HTMLElement).dispatchEvent(
      new MouseEvent('mousedown', { bubbles: true })
    );
    (bootstrapMenuButton as HTMLElement).dispatchEvent(
      new MouseEvent('mouseup', { bubbles: true })
